  ...
  useEffect(() => {
    const fetchLearningData = async () => {
      ...
      learningRecordsData.forEach(record => {
        if (record.status === 'completed' || record.clickCount > 0) {
      ...
      const progressData = Object.keys(categoryCounts).map(category => ({
        category,
        progress: Math.round((categoryCounts[category].completed / categoryCounts[category].total) * 100),
      ...
      const overallProgress = Math.round((totalCompleted / totalURLs) * 100);
